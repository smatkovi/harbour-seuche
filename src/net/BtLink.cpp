...
#  define SEUCHE_NO_BLUETOOTH 1
#endif

#ifndef SEUCHE_NO_BLUETOOTH
...
void RfcommSocket::connectToDevice(const QString& address, int channel)
{
    ...
    Q_UNUSED(address);
    Q_UNUSED(channel);
    fail(tr("Dieses Gerät hat kein Bluetooth"));
#else
    shutdown(false);
    unsigned char target[6];
    if (!parseAddress(address, target)) {
        fail(tr("Keine Bluetooth-Adresse"));
        return;
    }
...
bool RfcommServer::listen(int channel, QString* error)
{
    ...
    Q_UNUSED(channel);
    if (error)
        *error = tr("Dieses Gerät hat kein Bluetooth");
    return false;
#else
    close();
    if (!Bt::available()) {
        if (error)
            *error = tr("Bluetooth ist ausgeschaltet");
        return false;
    }
    m_fd = ::socket(kAfBluetooth, SOCK_STREAM, kBtprotoRfcomm);
    if (m_fd < 0) {
